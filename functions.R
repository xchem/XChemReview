...
createFragUploadFolder <- function(meta, target, copymaps=FALSE, mtz, configuration){
    ...
    # base_root <- sprintf('/dls/.../fragprep/pipeline_staging/%s/', prot)
    rootf <- file.path(configuration$prep_path, protsuffix)
    # rootf <- sprintf('/dls/.../fragprep/FragalysisUploadFolders/%s', protsuffix)
    basef <- file.path(rootf, prot)
    # basef <- sprintf('/dls/.../FragalysisUploadFolders/%s/%s', protsuffix, prot)
    align_dir <- file.path(basef, 'aligned/')
    # align_dir <- sprintf('/dls/.../FragalysisUploadFolders/%s/%s/aligned', protsuffix, prot)
    crys_dir <- file.path(basef, 'crystallographic/')
    # crys_dir <- sprintf('/dls/.../FragalysisUploadFolders/%s/%s/crystallographic', protsuffix, prot)
